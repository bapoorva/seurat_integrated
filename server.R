...
server <- function(input, output,session) {
  ...
  prjsumm <- reactive({
   ...
   maxdim=dim(scrna@reductions$pca@cell.embeddings)
   ...
   rownames(df)=c("Project name","Project Description","Organism","Total nummber of cells after filtering","Total number of genes","Dimension","","Cluster-wise number of cells",as.character(c.cnt$Var1))
   colnames(df)<- NULL
   return(df)
  })
  ...
  prjsumm2 <- reactive({
    scrna=fileload()
    meta=scrna@meta.data
    c.cnt= meta %>% group_by(orig.ident) %>% count(var_cluster) %>% spread(orig.ident,n)
    c.cnt=as.data.frame(c.cnt)
    # rownames(c.cnt)= c.cnt$var_cluster
...
#     colnames(markers)[5]="Percentage expressed in Cell Group 2"
  ...
  geplots = reactive({
    ...
    if(input$checkviolin2 ==T){
      if(input$splitviolin ==T){
      plot3=VlnPlot(object = scrna, features = input$geneid,pt.size=0,cols=cpallette, split.by = "orig.ident")
      }else{
  ...
  clusts= reactive({
    ...
    avgexp= avgexp$integrated %>% dplyr::select(input$selectcluster)
